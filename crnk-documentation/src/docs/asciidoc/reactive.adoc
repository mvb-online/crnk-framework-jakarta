...
2. `ReactiveModule.setWorkerScheduler(...)` allows to set the scheduler to use to invoke traditional repositories. By default `Schedulers.boundedElastic` is used where
